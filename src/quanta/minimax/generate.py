"""Autoregressive generation for MiniMax-M2.7 — seed the decode cache, then KV-cached decode.

Decode is inherently sequential (token *t+1* depends on *t*). MiniMax's decode attention is a
single-token cache step (:mod:`quanta.minimax.decode`) with no batched cache-fill, so the prompt is
consumed the same way generation is: one token at a time through the decode path, threading a
:class:`quanta.minimax.decode.MiniMaxCache`. Each step is numerically equivalent to the prefill block
at that absolute position (the cache path is gated == the prefill attention), so this is
parity-correct. The two coarse, bounded loops are the allowed kind (over prompt positions, then over
generated tokens) — one full forward per iteration, nothing per-element.
...
The sampling math mirrors the oMLX shim's ``_sample`` exactly (``quanta.shim.omlx``) and the sibling
DSV4 generator: greedy at ``temperature==0``; otherwise temperature, then top-k (k-th-largest
threshold), then top-p (nucleus), then min-p, then a seeded :func:`mx.random.categorical`.
``mx.random.key(seed)`` + per-step ``split`` makes sampling reproducible. One ``mx.eval`` per step
materializes the token + the grown cache.

(MiniMax has no native MTP head — see :mod:`quanta.minimax.runtime` — so there is no speculative
multi-token verify here; this is the plain one-token-per-step decoder.)
"""
...
def generate(
    ...
    *,
    ...
    Seeds the decode cache by stepping the prompt one token at a time (positions ``0..len-1``), then
    the bounded decode loop threads each new token back in at the next absolute position. ``model`` is
    a :class:`quanta.minimax.runtime.MiniMaxResidentModel` (or any object with ``.cfg``,
    ``.num_layers`` and the same single-token ``__call__(token_ids, caches=, offset=)`` contract).
    ``eos_id`` may be an ``int`` or a collection of stop ids; the loop also stops at ``max_new_tokens``
    so it can never run unbounded.

    ``cache`` is the per-stack decode cache; when ``None`` a fresh
    :class:`quanta.minimax.decode.MiniMaxCache` is built. Callers (and the model-free gate) may pass
    their own (e.g. a stub) to keep the run model-free.
    """
    ids = list(prompt_ids)
    if not ids:
