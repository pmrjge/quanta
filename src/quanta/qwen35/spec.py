...
def _mtp_top_w(logits_row: mx.array, width: int) -> list[int]:
    ...
    Used by :func:`spec_generate_tree`'s ``expand`` callable to seed the BFS tree-build. For a real
    Qwen3.5 vocab (``~150K``) and the canonical ``width ∈ {2, 4, 8}``, a single ``argsort`` is
    trivially cheap (~µs); we never materialize a per-vocab tensor anywhere else in the spec loop."""
    if width == 1:
...
def spec_generate_tree(model, mtp, embed: mx.array, head: mx.array, prompt_ids,
    ...
    model — with the decode cache (KV on full-attn layers + Gated-DeltaNet recurrent snapshots on
    linear layers) rolled back losslessly between paths via
    :meth:`quanta.qwen35.decode.Qwen35Cache.truncate`. The longest greedy-matching prefix across
    paths is committed (accepted drafts + bonus); a final commit-forward re-feeds the accepted
    drafts to advance the cache to the committed offset. Output is **bit-identical to plain greedy
    decode** for any MTP quality (CLAUDE.md rule 4) — the head only changes which candidate set is
    presented to verify, never which tokens commit.

    Why W-parallel chain verify and not the single-forward tree-causal-mask form (EAGLE-2)? Qwen3.5
    is a 3:1 hybrid: 45 Gated-DeltaNet *linear-attention* layers + 15 gated-GQA *full-attention*
    layers (60 total). A tree-causal mask only makes sense for full attention — the GDN layers
    process tokens via a recurrent state that has no causal-mask hook; feeding a BFS-linearized
    tree to the GDN layers in one forward would corrupt their recurrent state by mixing alternative
    branches into the summary. This entry point preserves losslessness for the hybrid stack by
    keeping each path's forward purely sequential (the recurrent state always sees one contiguous
    ...
    **Economics.** This implementation runs ``W ** D + 1`` main-model forwards per round
    ...
    At ``W=2, D=2`` that is 5 forwards; EAGLE-2 reports ``mean_accept ≈ 3.5–4.5`` at those settings,
    so PER ACCEPTED TOKEN this is ~3–4× the cost of :func:`spec_generate_k` (chained k=2: 1 forward
    at ``mean_accept ≈ 2.78``). USE :func:`spec_generate_k` FOR ACTUAL DECODE SPEEDUP ON HYBRID
    MODELS. This entry point is kept for (a) interface parity with pure-attention models (DSV4
    follow-on, where the tree-causal-mask form will eventually land); (b) lossless contract
    correctness — it's the form that admits a ``(W, D)`` tree on a hybrid architecture at all;
    (c) future batched-verify (B=``W ** D``) integration via :mod:`quanta.qwen35.batched_runtime`,
    which would collapse the ``W ** D`` sequential forwards into ONE batched forward and could net
    ...
    stop = _normalize_eos(eos_id)
    # max_rollback = depth + 1: per-path verify pushes ``depth+1`` new snapshots onto each
    # GDN-layer ring; the round-start (q+1) snapshot must remain reachable for the per-path
    # truncate. The +1 covers the commit-forward's extra commit-len snapshots in the same ring.
    ...
    while len(out) < max_new and not stopped:
        ...
        # children as ``[(child_token, child_hidden), ...]``. The MTP's post-block hidden is fed
        # to the next level (off-distribution past depth 1 — the head was trained on MAIN hidden;
        ...
        # committed offset. best_j == 0 commits only cur (the cache stays at q+1 — no extra forward).
        ...
        # Emit accepted drafts + the bonus (the main model's greedy at the first un-accepted
        # tree position along the best path). Mirrors :func:`spec_generate_k`'s emit pattern.
