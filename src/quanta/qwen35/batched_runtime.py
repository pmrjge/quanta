...
class Qwen35BatchedResidentModel:
    ...
    * ``.cfg``, ``.num_layers``, ``.embed_w``, ``.lm_head_w``, ``.norm_w`` — same handles as
    ...
    def from_inner(cls, layers, embed_w: mx.array, norm_w: mx.array, lm_head_w: mx.array,
        ...
        # routed-expert packing is a property of the passed layers (set at load via packed_experts),
        # not a construction choice here — detect it so the introspection surface mirrors __init__.
        self.packed_experts = bool(layers) and isinstance(layers[0].mlp.experts_gate_up, dict)
