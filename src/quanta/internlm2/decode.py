"""Per-layer KV cache state for incremental InternLM2.5-7B-Chat-1M decode.

The dense pure-attention architecture means decode state is just a list of per-layer
:class:`~quanta.internlm2.attention.KVCache` — no SSM state, no compressor pool, no indexer top-k.
:class:`InternLM2Cache` is the slim wrapper the runtime + generator + (future) spec-decoder pass
around; it stays in lock-step with :class:`~quanta.internlm2.attention.InternLM2Attention.__call__`
(every layer's KV cache grows by ``T`` per forward pass).

Mirrors the simpler subset of :mod:`quanta.qwen25.decode` (full-attention layers only) so the
...
class InternLM2Cache:
    """Decode-side cache for an InternLM2.5 resident model: one ``KVCache`` per decoder layer.
    ...
    * ``offset`` — current cached sequence length (consistent across layers).
    ...
    def __init__(self, cfg: InternLM2Config, *, quantized: bool = False, group_size: int = 64,
                 bits: int = 8) -> None:
        self.cfg = cfg
    ...
    def as_list(self) -> list[KVCache]:
        """The underlying per-layer KV caches in layer order — pass straight to ``InternLM2Model``."""
        return self.layers

    def truncate(self, length: int) -> None:
        """Lossless rollback: drop everything past ``length`` on every layer (spec-decode reject).

        Re-slicing along axis 2 mirrors :class:`quanta.qwen25.decode.Qwen25Cache.truncate`;
    ...
    def replicate(self, batch_size: int) -> "InternLM2Cache":
        """Zero-copy fan-out for tree spec-decode batched verify.

        Returns a new :class:`InternLM2Cache` whose per-layer KVs *broadcast* the current state
        across ``batch_size`` rows. MLX arrays are immutable so the replicated rows share storage
        with the source; a subsequent ``update`` on the replica creates new concatenated arrays
        per branch.
        """
        new = InternLM2Cache(self.cfg, quantized=self.quantized, group_size=self.group_size,
                              bits=self.bits)
