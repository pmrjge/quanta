"""Autoregressive generation for DeepSeek-V4-Flash — seed the decode cache, then KV-cached decode.

Decode is inherently sequential (token *t+1* depends on *t*). DSV4's decode attention is a single-token
stepper (:mod:`quanta.dsv4.decode`) with **no batched cache-fill**, so the prompt is consumed the same
way generation is: one token at a time through the decode path, threading a
:class:`quanta.dsv4.decode.DSV4Cache`. Each step is numerically equivalent to the prefill block at that
absolute position (the steppers are gated == the prefill attention), so this is parity-correct; it is
also the exact pattern Unit 3's own gate uses to seed the cache. The two coarse, bounded loops are the
...
The sampling math mirrors the oMLX shim's ``_sample`` exactly (``quanta.shim.omlx``): greedy at
``temperature==0``; otherwise temperature, then top-k (k-th-largest threshold), then top-p (nucleus),
then min-p, then a seeded :func:`mx.random.categorical`. ``mx.random.key(seed)`` + per-step ``split``
makes sampling reproducible. One ``mx.eval`` per step materializes the token + the grown cache.
"""
...
def generate(
    ...
    *,
    ...
    the bounded decode loop threads each new token back in at the next absolute position. ``model`` is a
    :class:`quanta.dsv4.runtime.DSV4ResidentModel` (or any object with ``.cfg``, ``.num_layers`` and the
    same single-token ``__call__(token_ids, caches=, offset=)`` contract). ``eos_id`` may be an ``int``
    or a collection of stop ids; the loop also stops at ``max_new_tokens`` so it can never run unbounded.

    ``cache`` is the per-stack decode cache; when ``None`` a fresh
    :class:`quanta.dsv4.decode.DSV4Cache` is built. Callers (and the model-free gate) may pass their own
    (e.g. a stub) to keep the run model-free.
    """
    ids = list(prompt_ids)
    if not ids:
    ...
    if cache is None:
        ...
        cache = DSV4Cache(model.num_layers)
