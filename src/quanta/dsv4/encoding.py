"""DeepSeek-V4 chat encoding/parsing — faithful vendored copy of the checkpoint's reference.

This is a byte-faithful copy of ``encoding/encoding_dsv4.py`` shipped with DeepSeek-V4-Flash (so the
runtime stays self-contained — no dependency on files under ``~/models`` at serve time). It is pure
Python (no torch/mlx) and is gated against the checkpoint's own gold fixtures in
``parity/dsv4_encoding_test.py``. Do not edit the reference functions below — fixes belong upstream.

quanta policy lives in the thin wrapper at the *end* of this file: :func:`encode_chat` hardcodes
``reasoning_effort="max"`` (and ``thinking_mode="thinking"``, required for the max-effort prefix to
apply) as defaults, so DeepSeek-V4-Flash runs at maximum reasoning effort unless explicitly overridden.
...
# ============================================================
# quanta policy wrapper — reasoning_effort="max" hardcoded default
# ============================================================

THINKING_MODE_DEFAULT: str = "thinking"
REASONING_EFFORT_DEFAULT: str = "max"
...
def encode_chat(
    ...
    *,
    ...
    reasoning_effort: Optional[str] = REASONING_EFFORT_DEFAULT,
    **kwargs: Any,
) -> str:
    """quanta entry point for DeepSeek-V4-Flash prompt encoding.

    Identical to :func:`encode_messages` but with ``reasoning_effort="max"`` and
    ``thinking_mode="thinking"`` as the **hardcoded defaults** (per project policy: run this model at
    maximum reasoning effort). The max-effort prefix only applies in thinking mode, so the two
    defaults move together. Pass ``reasoning_effort=None`` / ``thinking_mode="chat"`` to override.
    """
    assert reasoning_effort in ("max", "high", None), f"Invalid reasoning effort: {reasoning_effort}"
    return encode_messages(
        messages, thinking_mode=thinking_mode, reasoning_effort=reasoning_effort, **kwargs
    )
