...
from quanta.qwen35.spec import spec_generate, spec_generate_k
...
def _dummy_hidden() -> mx.array:
    """A ``[1,1,hidden]`` hidden the chained drafter can feed back in — content is unused by these
    stub drafters (which only consult ``next_ids``), but the shape must match what the real
    ``mtp_forward`` returns (``[B,T,hidden]``) so the chained spec loop's plumbing is exercised."""
...
def run() -> None:
    ...
    # (5) k=2 chained spec_generate_k — must also be bit-identical to greedy; perfect drafter at k=2
    ...
    spec_e3, _ = spec_generate_k(_StubMainModel(), _PerfectMTP(), embed, head, prompt,
                                 k=3, max_new=MAXN, eos_id=EOS)
    good = (spec_e3 == greedy_e and len(spec_e3) > 0 and spec_e3[-1] == EOS
            and EOS not in spec_e3[:-1])
    ok = ok and good
    print(f"  [{'OK' if good else 'FAIL'}] k=3 eos stops: spec==greedy={spec_e3 == greedy_e} "
          f"len={len(spec_e3)}")

    # k=2 rollback pattern: with a wrong drafter, every round rolls back exactly k tokens (the
    # full chained suffix is rejected each round). Verifies the cache truncate plumbing.
    rec_model_k2 = _StubMainModel()
    rec_cache_k2 = _StubCache()
    spec_generate_k(_WrapCache(rec_model_k2, rec_cache_k2), _WrongMTP(), embed, head, prompt,
                    k=2, max_new=MAXN, eos_id=None)
    rb_ok_k2 = bool(rec_cache_k2.truncations) and all(0 <= frm - to <= 2
                                                       for frm, to in rec_cache_k2.truncations)
    ok = ok and rb_ok_k2
    print(f"  [{'OK' if rb_ok_k2 else 'FAIL'}] k=2 rollback ≤2 tokens each round (lossless): "
          f"truncations={rec_cache_k2.truncations}")
