"""Lossless gate for Qwen3.5 W-parallel chain-verify tree drafting (#157 follow-on).

Validates :func:`quanta.qwen35.spec.spec_generate_tree` — the hybrid-safe form of tree drafting:
each round drafts a ``(width, depth)`` MTP tree, enumerates ``W ** D`` root-to-leaf paths, and
chain-verifies each (cache snapshot/restore between paths) so the GDN-recurrent state always sees
a contiguous chain. MODEL-FREE — a stub main model + stub MTP + stub cache; no checkpoint, no GPU,
a few KB of tensors — safe alongside another large GPU-resident job.
...
Asserts:
  ...
  (3) ``width=1`` degenerates to a chain of length ``depth`` AND produces identical output +
      ``mean_accept`` to :func:`quanta.qwen35.spec.spec_generate_k` with ``k=depth`` (the documented
...
Run:  ``uv run --with numpy python -m parity.qwen35_tree_spec_test``
"""
...
class _StubMainModel:
    """Deterministic stub of ``Qwen35ResidentModel``: greedy(t) = t + STEP, with MTP-feature capture.

    Honors the consumed contract ``(token_ids, *, caches, offset, capture_layers)`` →
    ``(logits [1,T,vocab], {last: hidden [T,hidden]})``. Advances the cache by ``len(token_ids)``
    so ``offset`` stays consistent after rollbacks, returns a deterministic per-position capture so
    the spec loop's feature plumbing is exercised. Records each call so the test can introspect."""
...
def _dummy_hidden() -> mx.array:
    ...
    real ``mtp_forward(...)`` return so the spec loop's plumbing is exercised."""
