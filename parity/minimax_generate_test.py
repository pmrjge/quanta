"""Gate: MiniMax-M2.7 sampling math + the generate loop bound — model-free, tiny tensors, ~0 GB.

Answers two questions without loading the model (safe to run while a big model is GPU-resident):

  (a) **Sampling math** (:func:`quanta.minimax.generate.sample_logits`): greedy == argmax;
      ...
      the same math the oMLX shim uses, so the engine and the standalone generator agree
      token-for-token.
  (b) **Generate-loop bound** (:func:`quanta.minimax.generate.generate`): driven by a STUB model (a
      tiny object exposing ``.cfg`` / ``.num_layers`` and the single-token ``__call__(token_ids,
      caches=, offset=)`` contract, returning fixed logits) — ``generate`` seeds the cache by stepping
      the prompt (asserted via recorded offsets) then decodes; an eos-less run is bounded by
    ...
    uv run --with numpy python -m parity.minimax_generate_test

Deferred (run later ON the M3 Ultra — loads the ~real artifact, NOT model-free):
    # from quanta.minimax.runtime import MiniMaxResidentModel
    # from quanta.minimax.generate import generate
    # m = MiniMaxResidentModel("/Users/pmrj/models/MiniMax-M2.7-quanta_<type>")
...
from quanta.minimax.generate import generate, sample_logits
...
class _StubCache:
    """A non-``None`` cache placeholder so ``generate`` skips building a real ``MiniMaxCache`` (keeps
    the gate model-free); the stub model never touches it."""
